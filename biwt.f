      ...
      real x(nmax)
c      real*8 dx(5000),dxb,dxs
      character filein*40

 1069 format(a)

c 1    write (*,"('Input data : '$)")
      read(*,1069) filein
      open(unit=1,file=filein,status='old',err=706)

c 2    write(*,"('Which col. (<10) : '$)")
      read *,icol
      if(icol.ge.10) goto 706

      n=0
      do i=1,nmax
         if(icol.eq.1) read(1,*,end=666) xval
         if(icol.eq.2) read(1,*,end=666) x1,xval
         if(icol.eq.3) read(1,*,end=666) x1,x2,xval
         if(icol.eq.4) read(1,*,end=666) x1,x2,x3,xval
         if(icol.eq.5) read(1,*,end=666) x1,x2,x3,x4,xval
         if(icol.eq.6) read(1,*,end=666) x1,x2,x3,x4,x5,xval
         if(icol.eq.7) read(1,*,end=666) x1,x2,x3,x4,x5,x6,xval
         if(icol.eq.8) read(1,*,end=666) x1,x2,x3,x4,x5,x6,x7,xval
         if(icol.eq.9) read(1,*,end=666) x1,x2,x3,x4,x5,x6,x7,x8,xval
         n=n+1
         x(n)=xval
c         dx(n)=dble(x(n))
      enddo
...
c      call xbiwt(x,n,xb,xs)
      call biwgt(x,n,xbn,xsn)
c      call dbiwgt(dx,n,dxb,dxs)

      open(unit=11,file='biwt.out',status='unknown')
c      print *,n,xbn,xsn
      write(11,*) n,xbn,xsn
      close(11)
 706  continue
