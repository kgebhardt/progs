 ...
 21   call qc1('Third image ','imar.def',file1)
      call qi1('Which extension ','imar.def',iext2)
      call savdef
      ...
      call ftgiou(im3,ier)
      iread=0
      call ftopen(im3,file1,iread,iblock,ier)
      if(ier.ne.0) then
         write(*,*) 'Error opening image : ',file1
         goto 21
      endif
      call ftmahd(im3,iext2,ihd,ier)
      call ftghpr(im3,2,simple,ibit,naxis,naxes,ipc,igc,extend,ier)
      if(naxes(1).gt.narrm1.or.naxes(2).gt.narrm2) then
         write(*,"('Arrays too small - make narrm bigger')")
         goto 706
      endif
      ncol=naxes(1)
      nrow=naxes(2)
      call ftg2de(im3,igc,-666.,narrm1,ncol,nrow,xd3,anyf,ier)
      call ftclos(im3,ier)
      call qr1('Value to cut on: ','imar.def',xcut)
 ...
 3    call qi1('Add (1), Sub (2), Mult (3), or Div (4) ',
     $     'imar.def',itype)
      ...
      if(itype.eq.1) then
         do j=1,nrow
            do i=1,ncol
               if(nint(xd(i,j)).ne.-666.and.nint(xd2(i,j)).ne.-666) then
         ...
         do j=1,nrow
            do i=1,ncol
               if(nint(xd(i,j)).ne.-666.and.nint(xd2(i,j)).ne.-666) then
                  ...
                  if(xd2(i,j).lt.xcut) xd(i,j)=xd3(i,j)
               else
                  xd(i,j)=-666
               endif
            enddo
...
c-- open the output file
      call ftclos(im1,ier)
      call ftinit(51,'imar.fits',iblock,ier)
