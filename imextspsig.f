...
c - this is the wavelength
      ...
      call ftgiou(im1,ier)
      call ftopen(im1,file1,iread,iblock,ier)
c - this is the spectrum wth no sky in it
      iext=16
      call ftmahd(im1,iext,ihd,ier)
      call ftghpr(im1,2,simple,ibit,naxis,naxes,ipc,igc,extend,ier)
      ncol=naxes(1)
...
c         print *,i,yp,ysky,sqrt(yp/ysky)
         if(yfp.gt.0.and.yf2f.gt.0.1.and.ysky.gt.0.and.yf2f.lt.10.) then
            ...
            ysky=ysky*convfac/wave(i)/ytp/yfp/yf2f
            write(11,1101) wave(i),yp,yflux,yfp,ytp,yf2f,ysky
         else
            write(11,1101) wave(i),0.,0.,0.,ytp,0.,0.
         endif
      enddo
      close(11)

 1101 format(1x,f8.2,1x,f9.2,1x,1pe11.2,3(1x,0pf7.3),1pe11.2)
 706  continue
      end
