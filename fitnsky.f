      ...
      parameter(nmax=10000,nlmax=1000,nca=17,ncol0=1100)
      ...
      real ydata(nmax)
      real xdw(ncol0,35000),xds(ncol0,35000),xde(ncol0,35000)
      integer naxes(2)
      ...
      open(unit=1,file='lines.in',status='old')
      read(1,*) wave0
      close(1)

      nadd=5

      open(unit=3,file='list.in',status='old')
      do ifile=1,1000
      read(3,*,end=777) file1
...
c - this is the wavelength                                                                                                         
      ...
      ncol=naxes(1)
      nrow=max(1,naxes(2))
      call ftg2de(im1,igc,0.,ncol0,ncol,nrow,xdw,anyf,ier)
      call ftclos(im1,ier)
...
c - this is the spectrum
      ...
      ncol=naxes(1)
      nrow=max(1,naxes(2))
      call ftg2de(im1,igc,0.,ncol0,ncol,nrow,xds,anyf,ier)
      call ftclos(im1,ier)

c - this is the error spectrum
      im1=0
      ier=0
      iread=0
      call ftgiou(im1,ier)
      ...
      ncol=naxes(1)
      nrow=max(1,naxes(2))
      call ftg2de(im1,igc,0.,ncol0,ncol,nrow,xde,anyf,ier)
      call ftclos(im1,ier)
...
c         fluxe(n)=fluxerr
     ...
     $           nint(flux(i)).ne.-666) then
               ...
               ymin=min(ymin,y(nt))
               ymax=max(ymax,y(nt))
               if(ye(nt).ge.0.and.ye(nt).lt.1.e6) then
                  iskip=0
               else
                  iskip=1
                  goto 965
         ...
         sigg=sqrt(sigg*sigg)
         if(sigg.gt.7.) goto 965
c         write(*,1101) irow0,amp,sigg,wfit0,file1
         write(11,1101) irow0,amp,sigg,wfit0,file1
         goto 966
 965     continue
c         write(*,1101) irow0,0.,0.,0.,file1
         write(11,1101) irow0,0.,0.,0.,file1
 966     continue
      enddo
 ...
 1101 format(i3,1x,f9.1,1x,f6.2,1x,f8.2,3x,a20)
      call pgend
      end
